...
export const OpMap = {
  ...
  [Op.between]: (key: string, value: any): SqlBind => {
    if (!Array.isArray(value)) {
      throw new Error("between need Array");
    }
    if (value.length !== 2) {
      throw new Error("between need Array length is 2");
    }
    return {
      sql: `${key} BETWEEN ? AND ?`,
      bind: value,
  ...
  [Op.in]: (key: string, value: any): SqlBind => {
    if (!Array.isArray(value)) {
      throw new Error("IN need Array");
    }
    if (value.length <= 0) {
      throw new Error("IN need Array length gt 0");
    }
    return {
  ...
  [Op.notIn]: (key: string, value: any): SqlBind => {
    if (!Array.isArray(value)) {
      throw new Error("NOT IN need Array");
    }
    if (value.length <= 0) {
      throw new Error("NOT IN need Array length gt 0");
    }
    return {
